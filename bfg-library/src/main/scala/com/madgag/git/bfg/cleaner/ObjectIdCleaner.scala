...
class ObjectIdCleaner(config: ObjectIdCleaner.Config, objectDB: ObjectDatabase, implicit val revWalk: RevWalk) extends CleaningMapper[ObjectId] {
  ...
  val commitMemo = MemoUtil.concurrentAsyncCleanerMemo(protectedObjectCensus.fixedObjectIds)
  val tagMemo = MemoUtil.concurrentAsyncCleanerMemo(protectedObjectCensus.fixedObjectIds)

  val treeMemo = MemoUtil.concurrentBlockingCleanerMemo(protectedObjectCensus.fixedObjectIds)
